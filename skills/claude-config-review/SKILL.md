...
| Secrets | `.env` (check password strength, not committed but audited locally) |
| Hooks | `hooks/*` |
...
- Weak passwords: any password in `.env` or config files that is NOT generated by a strong password generator. A strong password must be 24+ chars, mixed case, digits, and special chars, with high entropy (looks random, not human-crafted like `Codeforces@2026`). If a weak password is found, flag it as CRITICAL and tell the user to regenerate with `openssl rand -base64 24`
